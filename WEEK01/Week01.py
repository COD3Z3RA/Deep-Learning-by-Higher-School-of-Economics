...
In logistic regression the optimal parameters ww are found by cross-entropy minimization:

Loss for one sample:
l(xi,yi,w)=−[yi⋅logP(yi=1|xi,w)+(1−yi)⋅log(1−P(yi=1|xi,w))]
l(xi,yi,w)=−[yi⋅logP(yi=1|xi,w)+(1−yi)⋅log(1−P(yi=1|xi,w))]
Loss for many samples:
L(X,y⃗ ,w)=1ℓ∑i=1ℓl(xi,yi,w)
L(X,y→,w)=1ℓ∑i=1ℓl(xi,yi,w)
...
∇wL=1ℓ∑i=1ℓ∇wl(xi,yi,w)
∇wL=1ℓ∑i=1ℓ∇wl(xi,yi,w)
We won't be giving you the exact formula this time — instead, try figuring out a derivative with pen and paper.

As usual, we've made a small test for you, but if you need more, feel free to check your math against finite differences (estimate how LL changes if you shift ww by 10−510−5 or so).
...
Mini-batch SGD
Stochastic gradient descent just takes a random batch of mm samples on each iteration, calculates a gradient of the loss on it and makes a step:
wt=wt−1−η1m∑j=1m∇wl(xij,yij,wt)
wt=wt−1−η1m∑j=1m∇wl(xij,yij,wt)
...
<matplotlib.figure.Figure at 0x7fa0744dcd68>
# use output of this cell to fill answer field 
​
...
Submitted to Coursera platform. See results on assignment page!
SGD with momentum
Momentum is a method that helps accelerate SGD in the relevant direction and dampens oscillations as can be seen in image below. It does this by adding a fraction αα of the update vector of the past time step to the current update vector.


νt=ανt−1+η1m∑j=1m∇wl(xij,yij,wt)
νt=ανt−1+η1m∑j=1m∇wl(xij,yij,wt)
wt=wt−1−νt
wt=wt−1−νt
...
<matplotlib.figure.Figure at 0x7fa0740df160>
# use output of this cell to fill answer field 
​
...
<matplotlib.figure.Figure at 0x7fa074178cc0>
# use output of this cell to fill answer field 
